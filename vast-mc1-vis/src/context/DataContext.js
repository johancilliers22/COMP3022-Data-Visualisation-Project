  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // If there are no reports but the original data file exists, try loading it directly
        if (result.reports.length === 0) {
          logger.debug('DataContext: No reports found in processed data, attempting to load CSV directly...');
          setLoadingProgress('No reports found in processed data, attempting to load from CSV...');
          
          try {
            // Try to load the CSV file directly
            const csvResponse = await fetch('/data/mc1-reports-data.csv');
            if (csvResponse.ok) {
              logger.debug('DataContext: CSV file found, processing...');
              setLoadingProgress('Loading CSV data...');
              
              // Import the Papa Parse library dynamically to parse CSV
              ...
              const parsedData = await new Promise((resolve) => {
                Papa.default.parse(csvText, {
                  ...
                  error: (error) => {
                    logger.error('DataContext: Error parsing CSV:', error);
                    resolve([]);
                  }
                });
              });
              ...
              if (Array.isArray(parsedData) && parsedData.length > 0) {
                logger.debug(`DataContext: Successfully parsed ${parsedData.length} records from CSV`);
                
                // Log the structure of the first CSV row to understand its format
                if (parsedData.length > 0) {
                  logger.debug('DataContext: CSV first row example:', parsedData[0]);
                }
                
                // Define expected category names
                const categoryFields = ['buildings', 'medical', 'power', 'roads_and_bridges', 'sewer_and_water', 'shake_intensity'];
                
                // Basic data structure from CSV
                const reports = [];
                
                // Check CSV format: is it one category per row or multiple categories per row?
                const firstRow = parsedData[0] || {};
                const hasMultipleCategories = categoryFields.some(cat => firstRow[cat] !== undefined);
                
                if (hasMultipleCategories) {
                  // Format where each row has multiple category columns
                  logger.debug('DataContext: CSV format has multiple category columns per row');
                  
                  parsedData.forEach(row => {
                    if (!row || !row.time || !row.location) return;
                    
                    // Create a base report without category specifics
                    const baseReport = {
                      time: row.time,
                      location: row.location.toString(),
                      locationName: row.loc_name || row.location_name || `Location ${row.location}`,
                      certainty_level: row.certainty_level || 'medium'
                    };
                    
                    // For each category field, create a separate report
                    categoryFields.forEach(category => {
                      if (row[category] !== undefined) {
                        const reportValue = 
                          typeof row[category] === 'number' ? row[category] : 
                          typeof row[category] === 'string' ? parseFloat(row[category]) : 
                          null;
                        
                        if (reportValue !== null && !isNaN(reportValue)) {
                          reports.push({
                            ...baseReport,
                            category,
                            reportValue,
                            // Add confidence intervals if available
                            ci_lower: row[`${category}_ci_lower`] || reportValue - 0.5,
                            ci_upper: row[`${category}_ci_upper`] || reportValue + 0.5
                          });
                        }
                      }
                    });
                  ...
                  // Standard format where each row has a single category
                  logger.debug('DataContext: CSV format has single category per row');
                  
                  // Filter and process each row
                  parsedData.filter(row => row && row.time && row.location).forEach(row => {
                    // Try to determine the category
                    let category = row.category;
                    let reportValue = row.report_value || row.reportValue || row.value;
                    
                    // If no category field, try to determine from other fields
                    if (!category) {
                      // Look for a field matching category names
                      ...
                      // If still no category, try damage_type
                      if (!category && row.damage_type) {
                        category = row.damage_type;
                      }
                    }
                    
                    // Skip if no category identified
                    if (!category) return;
                    
                    // Ensure reportValue is a number
                    if (typeof reportValue !== 'number') {
                      reportValue = parseFloat(reportValue);
                      if (isNaN(reportValue)) reportValue = 0;
                    }
                    ...
                    reports.push({
                      ...
                      reportValue,
                      certainty_level: row.certainty_level || 'medium',
                      ci_lower: row.ci_lower || reportValue - 0.5,
                      ci_upper: row.ci_upper || reportValue + 0.5
                    });
                  });
                }
                ...
                reports.forEach(report => {
                  const locationId = report.location.toString();
                  if (!neighborhoodGroups[locationId]) {
                    neighborhoodGroups[locationId] = [];
                  }
                  neighborhoodGroups[locationId].push(report);
                });
                
                // Update the result
                result.reports = reports;
                result.neighborhoodGroups = neighborhoodGroups;
                
                logger.debug(`DataContext: Added ${reports.length} reports from CSV to the dataset`);
                logger.debug(`DataContext: Created ${Object.keys(neighborhoodGroups).length} neighborhood groups`);
                ...
                logger.debug('DataContext: Category distribution:', categoryCount);
              }
            }
          } catch (csvError) {
            logger.error('DataContext: Error loading CSV data:', csvError);
          }
        }
